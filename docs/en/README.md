...
Replaces the default user/password login by an authentication on an OAUTH server, with
OIDC support and group synchronisation.

## Configuration

After installing, add the following block to your `wakka.config.php` and adjust it to
your needs.
...
`logincas` does the same job against a CAS server, and `loginldap` against an LDAP
directory. Only one authentication extension can be active at a time: YesWiki loads
extensions in alphabetical order and the last one loaded takes over.
